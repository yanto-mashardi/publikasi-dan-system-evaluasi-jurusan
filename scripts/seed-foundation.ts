...
const ROLE_GRANTS:Record<string,string[]>={
 ...
 KAPRODI:["data.create","data.update","program.read","program.update","curriculum.manage","kpi.measure","evidence.upload","evaluation.create","finding.create","recommendation.create","followup.execute","approval.final","evaluation.approve","internal.read","resources.read","resources.contribute","accreditation.read","accreditation.assign","accreditation.assess","accreditation.approve"],
 ...
 KAJUR:["data.create","data.update","approval.final","evaluation.approve","publication.approve","publication.execute","report.approve","internal.read","resources.read","resources.manage","accreditation.read","accreditation.approve","accreditation.publish"],
...
async function main(){
 ...
 if(!org){const code=process.env.INITIAL_ORGANIZATION_CODE??"CONTOH-JURUSAN",name=process.env.INITIAL_ORGANIZATION_NAME??"Jurusan Contoh";const x=await db.insert(organizations).values({type:"UPPS",code,name});[org]=await db.select().from(organizations).where(eq(organizations.id,Number(x[0].insertId))).limit(1);}
 ...
 const existingPrograms=await db.select({id:studyPrograms.id}).from(studyPrograms).limit(1);
 if(!existingPrograms.length){const demoPrograms=[{code:process.env.INITIAL_PROGRAM_1_CODE??"CONTOH-PRODI-1",name:process.env.INITIAL_PROGRAM_1_NAME??"Program Studi Contoh 1"},{code:process.env.INITIAL_PROGRAM_2_CODE??"CONTOH-PRODI-2",name:process.env.INITIAL_PROGRAM_2_NAME??"Program Studi Contoh 2"}];for(const program of demoPrograms)await db.insert(studyPrograms).values({organizationId:org.id,level:"D3",...program});}
