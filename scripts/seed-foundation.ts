...
async function main(){
 ...
 const demoPrograms=[{code:process.env.INITIAL_PROGRAM_1_CODE??"CONTOH-PRODI-1",name:process.env.INITIAL_PROGRAM_1_NAME??"Program Studi Contoh 1"},{code:process.env.INITIAL_PROGRAM_2_CODE??"CONTOH-PRODI-2",name:process.env.INITIAL_PROGRAM_2_NAME??"Program Studi Contoh 2"}];
 for(const program of demoPrograms){const[found]=await db.select().from(studyPrograms).where(eq(studyPrograms.code,program.code)).limit(1);if(!found)await db.insert(studyPrograms).values({organizationId:org.id,level:"D3",...program});}
