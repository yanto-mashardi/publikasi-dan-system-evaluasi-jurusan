...
export async function provisionTenantDatabase(databaseName:string,seed:TenantSeed){
  ...
  try{
    ...
    for(const table of ["roles","permissions","role_permissions","role_settings","publication_policies","evaluation_modules"] as const)if(available.has(table))await connection.query(`INSERT IGNORE INTO \`${target}\`.\`${table}\` SELECT * FROM \`${source}\`.\`${table}\``);
    const frameworkCodes=[...new Set(seed.programs.map(program=>program.frameworkCode).filter((value):value is string=>Boolean(value)))];
    for(const frameworkCode of frameworkCodes){
      const[frameworkRows]=await connection.execute<mysql.RowDataPacket[]>(`SELECT id FROM \`${source}\`.accreditation_frameworks WHERE code=? AND lifecycle_status='ACTIVE' ORDER BY version_number DESC LIMIT 1`,[frameworkCode]);
      if(!frameworkRows.length)throw new Error(`Instrumen aktif ${frameworkCode} tidak ditemukan di Master.`);
      const frameworkId=Number(frameworkRows[0].id);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_agencies SELECT agency.* FROM \`${source}\`.accreditation_agencies agency INNER JOIN \`${source}\`.accreditation_frameworks framework ON framework.agency_id=agency.id WHERE framework.id=?`,[frameworkId]);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_frameworks SELECT * FROM \`${source}\`.accreditation_frameworks WHERE id=?`,[frameworkId]);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_clusters SELECT * FROM \`${source}\`.accreditation_clusters WHERE framework_id=?`,[frameworkId]);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_criteria SELECT * FROM \`${source}\`.accreditation_criteria WHERE framework_id=?`,[frameworkId]);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_indicators SELECT * FROM \`${source}\`.accreditation_indicators WHERE framework_id=?`,[frameworkId]);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_indicator_variables SELECT variable.* FROM \`${source}\`.accreditation_indicator_variables variable INNER JOIN \`${source}\`.accreditation_indicators indicator ON indicator.id=variable.indicator_id WHERE indicator.framework_id=?`,[frameworkId]);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_scoring_rubrics SELECT rubric.* FROM \`${source}\`.accreditation_scoring_rubrics rubric INNER JOIN \`${source}\`.accreditation_indicators indicator ON indicator.id=rubric.indicator_id WHERE indicator.framework_id=?`,[frameworkId]);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_indicator_clusters SELECT link.* FROM \`${source}\`.accreditation_indicator_clusters link INNER JOIN \`${source}\`.accreditation_indicators indicator ON indicator.id=link.indicator_id WHERE indicator.framework_id=?`,[frameworkId]);
      await connection.execute(`INSERT IGNORE INTO \`${target}\`.accreditation_evidence_requirements SELECT evidence.* FROM \`${source}\`.accreditation_evidence_requirements evidence INNER JOIN \`${source}\`.accreditation_indicators indicator ON indicator.id=evidence.indicator_id WHERE indicator.framework_id=?`,[frameworkId]);
    }
