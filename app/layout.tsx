...
export default async function RootLayout({children}:Readonly<{children:React.ReactNode}>){const [session,identity]=await Promise.all([getSession(),getSiteIdentity()]);const roles=session?.roles.map(role=>roleNames[role]??role).join(", ");return <html lang="id"><body><nav className="nav"><Link className="brand" href="/"><span>{identity.organization?.type??"JURUSAN"}</span> {identity.displayName}</Link><div className="navlinks"><Link href="/">Beranda</Link><Link href="/akreditasi">Akreditasi</Link><Link href="/berita">Berita</Link>{session?<><Link className="session-user" href="/internal"><span>{roles}</span><b>{session.name}</b></Link><form action="/api/auth/logout" method="post"><button className="nav-logout" type="submit">Keluar</button></form></>:<Link className="nav-login" href="/internal/login">Masuk Pengelola</Link>}</div></nav>{children}</body></html>}
