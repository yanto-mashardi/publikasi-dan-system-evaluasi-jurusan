...
import { deleteTenantDatabase,getTenantDatabaseDetails,provisionTenantDatabase,resetTenantAdminPassword,syncTenantDatabaseSchema } from "@/src/services/tenant-provisioning";
...
import { assertLocalTenantTargetAvailable,createLocalTenantInstance,deleteLocalTenantInstance,syncLocalTenantInstance } from "@/src/services/local-tenant-instance";
...
export async function PATCH(req:Request){
  const auth=await guard();if(auth.response)return auth.response;
  const parsed=z.discriminatedUnion("action",[z.object({id:z.number().int().positive(),action:z.literal("RESET_ADMIN_PASSWORD"),email:z.string().email(),newPassword:z.string().min(10).max(128)}),z.object({id:z.number().int().positive(),action:z.literal("SYNC_LOCAL_CODE")})]).safeParse(await req.json().catch(()=>null));if(!parsed.success)return NextResponse.json({error:"Permintaan pembaruan Tenant tidak valid."},{status:400});
  ...
  if(parsed.data.action==="SYNC_LOCAL_CODE"){if((configuration as {deploymentTarget?:string}).deploymentTarget!=="LOCAL"&&tenant.deploymentStatus!=="TENANT_LOCAL_READY")return NextResponse.json({error:"Sinkronisasi folder hanya tersedia untuk Tenant lokal."},{status:409});try{const result=await syncLocalTenantInstance({name:tenant.name,code:tenant.code,databaseName:tenant.databaseName}),database=await syncTenantDatabaseSchema(tenant.databaseName);await audit({actorId:auth.session!.userId,action:"SYNC_LOCAL_TENANT_CODE_AND_SCHEMA",subjectType:"TENANT_APPLICATION",subjectId:tenant.id,after:{...result,...database}});return NextResponse.json({message:`Kode dan struktur database ${tenant.name} berhasil diperbarui.`,...result,database});}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Kode dan database Tenant lokal tidak dapat diperbarui."},{status:409});}}
