...
export async function PATCH(req:Request){
  ...
  if(parsed.data.action==="SYNC_LOCAL_CODE"){if((configuration as {deploymentTarget?:string}).deploymentTarget!=="LOCAL"&&tenant.deploymentStatus!=="TENANT_LOCAL_READY")return NextResponse.json({error:"Sinkronisasi folder hanya tersedia untuk Tenant lokal."},{status:409});try{const result=await syncLocalTenantInstance({name:tenant.name,code:tenant.code,databaseName:tenant.databaseName});await audit({actorId:auth.session!.userId,action:"SYNC_LOCAL_TENANT_CODE",subjectType:"TENANT_APPLICATION",subjectId:tenant.id,after:result});return NextResponse.json({message:`Kode aplikasi ${tenant.name} berhasil diperbarui di ${result.folderPath}.`,...result});}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Kode Tenant lokal tidak dapat diperbarui."},{status:409});}}
