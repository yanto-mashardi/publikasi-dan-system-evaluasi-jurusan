...
import { subjectScopeAllows } from "@/src/services/subject-scope";
...
export async function POST(req:Request){const s=await getSession();if(!s)return NextResponse.json({error:"Unauthorized"},{status:401});if(!can(s,"publication.execute"))return NextResponse.json({error:"Forbidden"},{status:403});const p=input.safeParse(await req.json());if(!p.success)return NextResponse.json({error:p.error.flatten()},{status:400});if(!(await subjectScopeAllows(s,p.data.subjectType,p.data.subjectId)))return NextResponse.json({error:"Publisher tidak memiliki scope terhadap objek ini."},{status:403});try{const id=await publishApproved({...p.data,publishedBy:s.userId});await audit({actorId:s.userId,action:"PUBLISH",subjectType:p.data.subjectType,subjectId:p.data.subjectId,after:{publicationId:id}});return NextResponse.json({publicationId:id,status:"PUBLISHED"},{status:201});}catch(e){return NextResponse.json({error:e instanceof Error?e.message:"Publication failed"},{status:409});}}
