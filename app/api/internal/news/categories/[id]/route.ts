...
export async function DELETE(_:Request,{params}:{params:Promise<{id:string}>}){const s=await getSession();if(!s)return NextResponse.json({error:"Unauthorized"},{status:401});if(!can(s,"news.manage"))return NextResponse.json({error:"Forbidden"},{status:403});const id=await idOf(params);if(!id)return NextResponse.json({error:"Invalid id"},{status:400});const db=requireDb();const[before]=await db.select().from(newsCategories).where(eq(newsCategories.id,id)).limit(1);if(!before)return NextResponse.json({error:"Not found"},{status:404});const[article]=await db.select({id:newsArticles.id}).from(newsArticles).where(eq(newsArticles.categoryId,id)).limit(1);if(article)return NextResponse.json({error:"Kategori tidak dapat dihapus karena masih digunakan berita."},{status:409});await db.delete(newsCategories).where(eq(newsCategories.id,id));await audit({actorId:s.userId,action:"DELETE_NEWS_CATEGORY",subjectType:"NEWS_CATEGORY",subjectId:id,before,after:null});return NextResponse.json({id,deleted:true});}
