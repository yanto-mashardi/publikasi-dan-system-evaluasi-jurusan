...
export async function POST(req:Request){const s=await getSession();if(!s)return NextResponse.json({error:"Unauthorized"},{status:401});if(!can(s,"master.manage"))return NextResponse.json({error:"Forbidden"},{status:403});const p=input.safeParse(await req.json());if(!p.success)return NextResponse.json({error:p.error.flatten()},{status:400});const db=requireDb();const active=await db.select({id:organizations.id}).from(organizations).where(eq(organizations.status,"ACTIVE"));if(active.length)return NextResponse.json({error:"Satu instalasi hanya untuk satu Jurusan. Ubah identitas Jurusan yang ada, atau buat instalasi aplikasi baru."},{status:409});const [existing]=await db.select({id:organizations.id}).from(organizations).where(eq(organizations.code,p.data.code.toUpperCase())).limit(1);if(existing)return NextResponse.json({error:"Kode organisasi sudah digunakan."},{status:409});const x=await db.insert(organizations).values({...p.data,parentId:null,code:p.data.code.toUpperCase(),status:"ACTIVE"});const id=Number(x[0].insertId);await audit({actorId:s.userId,action:"CREATE_ORGANIZATION",subjectType:"ORGANIZATION",subjectId:id,after:p.data});return NextResponse.json({id},{status:201});}
