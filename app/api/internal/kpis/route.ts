...
export async function POST(req:Request){const s=await getSession();if(!s)return NextResponse.json({error:"Unauthorized"},{status:401});if(!(can(s,"data.create")||can(s,"program.update")))return NextResponse.json({error:"Forbidden"},{status:403});const parsed=input.safeParse(await req.json());if(!parsed.success)return NextResponse.json({error:parsed.error.flatten()},{status:400});const db=requireDb();const r=await db.insert(kpis).values({...parsed.data,lifecycleStatus:"DRAFT"});const newId=Number(r[0].insertId);await audit({actorId:s.userId,action:"CREATE",subjectType:"KPI",subjectId:newId,after:parsed.data});return NextResponse.json({id:newId,status:"DRAFT"},{status:201});}
